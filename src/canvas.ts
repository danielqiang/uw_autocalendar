...
export default class Canvas {
    ...
    constructor() {
        ...
        this.url = 'https://canvas.uw.edu/api/v1/courses';
    }
     ...
     * (this requires communication between foreground and background scripts, very hard)
    ...
    async check_login_auth(): Promise<boolean> {
        return await this.session.get(this.url).then(resp => {
    ...
    async get_ics(): Promise<any> {
        ...
        if (is_authenticated) {
            ...
            let thirteen_weeks_ago = new Date((new Date()).getTime() - (1000*60*60*24*7*13));

            // While there are more classes to look at.
            while (flag) {
                await this.session.get(this.url + "?page=" + page_num)
                    .then(resp=>resp.json())
                    .then(resp => {
            ...
            for (let course_index in cur_course_cal_urls){
                ...
                let calendar = await this.session.get(ics).then(response => response.body);
