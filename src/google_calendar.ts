...
export class GoogleCalendarEvent {
    ...
    constructor(start: Date, end: Date, timezone: CalendarTimeZone = CalendarTimeZone.US_LA, summary?: string, description?: string) {
        this.start = start
        this.end = end
...
export default class GoogleCalendar {
    ...
    async download_events(calendarId: string = "primary"): Promise<any> {
        ...
        return this.session.get(url, {headers: headers}).then(resp => resp.json())
    }

    async create_event(event: GoogleCalendarEvent, calendarId: string = "primary"): Promise<any> {
        const url = `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events`;
        const headers = await this.default_headers();
        const body = event.to_json();

        return this.session.post(url, {headers: headers, body: body}).then(resp => resp.json())
    }
